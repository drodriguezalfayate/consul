class Verification::Residenceva
  ...
  def document_number_uniqueness
    if ( (document_type.to_s == "1" || document_type.to_s == "3") )
      ...
      errors.add(:document_number, I18n.t('errors.messages.taken')) if User.where( document_number: document_number ).where("id != ?", user.id ).any?
    end
  end
  ...
  def same_date_of_birth?
    true #@census_api_response.date_of_birth == date_of_birth.strftime("%Y%m%d%H%M%S")
  end
